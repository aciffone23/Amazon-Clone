...
import { clearCart, fetchCart, removeFromCart, updateCart } from '../../store/cart';
import { fetchAllProducts } from '../../store/product';
import './CartListings.css';
import { Link } from 'react-router-dom';
import emptyCartImg from '../../imgs/logoImgs/emptyCart.png'
    ...
    return (
      <div className="cart-listings-container">
        ...
        <img src={emptyCartImg} alt="Empty Cart" className="empty-cart-img" />
          ...
          <Link to="/products" >
            <div className="all-products-link">View all products</div>
          </Link>
        </div>
      </div>
    );
    ...
    <div className="cart-listings-container">
        ...
        {cartItems.map((cartItem, index) => (
          <div key={`${cartItem.id}-${index}`} className="cart-product-box">
            ...
            <div className="cart-item-details">
              ...
              <p className="delete-item-link" onClick={() => handleDeleteItem(cartItem.id)}>
                <span className="delete-item-text">Delete</span>
              </p>
            </div>
          </div>
        ))}
      ...
      <div className="cart-summary">
        <p className="cart-total-quantity">
          Subtotal ({totalQuantity} items):<span className="cart-total-price">${totalPrice}</span>
        </p>
      </div>
    </div>
  );
